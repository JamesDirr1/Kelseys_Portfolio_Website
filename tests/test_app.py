...
def test_database_setup_failed_connection_exception(test_app_client_and_mocks):
    ...
    assert mock_root.try_connection.call_count == 2
    mock_logger.error.assert_any_call("----Could not connect to the database----\n Connection Failed")
...
def test_category_list(test_app_client_and_mocks):
    ...
    test_list = [Category("test", 1, 1), Category("test_2", 2, 2), Category("About", 0, 3)]

    mock_view_user.get_all_categories.return_value = [Category("test", 1, 1), Category("test_2", 2, 2)]
